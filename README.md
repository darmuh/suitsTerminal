...
### Suits Terminal Menu:  
 ...
 - The ``suits`` command will bring up an interactive menu that can be navigated by a configurable set of keys (defaults to arrow keys).  
	- Please note that any other mod that uses the arrow keys for functionality could interfere with this menu.  
 - Keywords can be customized using the ``Main Keywords``, ``Listing Keywords``, and ``Favorites Keywords`` configuration items.  
	- A blank value for any of these configuration items will mean the command will not be created. Except for ``Main Keywords`` which will create a command under the default word ``suits`` when left blank.
 - The ``Start Page`` configuration item determines what page the main menu keyword opens to.  
